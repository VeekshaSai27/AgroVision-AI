...
#### Option D: Flask Frontend (HTML + Flask)
If you prefer a simple HTML + Flask frontend (replaces Streamlit UI), a lightweight frontend is available in `flask_frontend.py` which proxies uploads to the FastAPI backend.
...
3. Run the Flask frontend (Terminal 2):

   ```cmd
   set DETECT_API_URL=http://localhost:8000
...
The Flask frontend forwards the uploaded image to the `/disease-detection-file` FastAPI endpoint and renders the returned JSON as an HTML result card.
